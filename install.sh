...
# RPC Latency Tracker Installation Script
# This script installs the RPC Latency Tracker application and sets it up as a service

# Exit on error
set -e

echo "====================================="
echo "  RPC Latency Tracker Installer"
echo "====================================="
echo ""
...
read -p "Enter installation directory [/opt/rpc-latency-tracker]: " INSTALL_DIR
INSTALL_DIR=${INSTALL_DIR:-/opt/rpc-latency-tracker}

# Get service user
read -p "Enter user to run the service [$(logname)]: " SERVICE_USER
...
cat > latency_tracker.py << 'EOF'

# Main python app pasted here 
...
# --- FastAPI App ---
app = FastAPI(title="Dual-Tier RPC Latency Tracker")
...
# Add the complete Python application code to latency_tracker.py
# (This would be the complete code we developed earlier)

# Create systemd service file
echo "Creating systemd service..."
cat > /etc/systemd/system/rpc-latency-tracker.service << EOF
[Unit]
Description=RPC Latency Tracker Service
After=network.target

[Service]
User=$SERVICE_USER
WorkingDirectory=$INSTALL_DIR
ExecStart=$INSTALL_DIR/venv/bin/python3 $INSTALL_DIR/latency_tracker.py
Restart=on-failure
RestartSec=5s
...
chmod +x $INSTALL_DIR/latency_tracker.py
...
This is the RPC Latency Tracker service, installed on $(date).
...
Start the service:
\`\`\`
sudo systemctl start rpc-latency-tracker
\`\`\`

Stop the service:
\`\`\`
sudo systemctl stop rpc-latency-tracker
\`\`\`

Restart the service:
\`\`\`
sudo systemctl restart rpc-latency-tracker
\`\`\`

Check service status:
\`\`\`
sudo systemctl status rpc-latency-tracker
\`\`\`

View logs:
\`\`\`
sudo journalctl -u rpc-latency-tracker -f
\`\`\`
...
- /simple-latency - Simple latency data
- /advanced-latency - Advanced latency data
- /providers - List of configured providers
- /methods - List of available methods

## Troubleshooting

If you encounter issues, check the logs using journalctl.
EOF

# Enable and start the service
echo "Enabling and starting service..."
systemctl daemon-reload
systemctl enable rpc-latency-tracker.service
systemctl start rpc-latency-tracker.service

# Check if service started successfully
if systemctl is-active --quiet rpc-latency-tracker.service; then
  echo "Service started successfully!"
else
  echo "Service failed to start. Check logs with: journalctl -u rpc-latency-tracker -n 50"
fi
...
echo "====================================="
echo ""
echo "RPC Latency Tracker has been installed to: $INSTALL_DIR"
echo "Service is running as: $SERVICE_USER"
echo ""
echo "Configuration file: $INSTALL_DIR/config.yaml"
echo "Database file: $INSTALL_DIR/latency_tracker.db"
echo ""
...
echo "  sudo systemctl status rpc-latency-tracker"
echo "  sudo systemctl restart rpc-latency-tracker"
echo "  sudo systemctl stop rpc-latency-tracker"
echo ""
